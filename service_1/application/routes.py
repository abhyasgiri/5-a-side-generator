...
@app.route("/")
def index():
    league_response = requests.get("http://football-cards_service-2:5002/league")
    pack_response = requests.get("http://football-cards_service-3:5003/pack", data=league_response.text)

    information = {'league': league_response.json()["league"] , 'pack': pack_response.json()["pack"]}

    player_response = requests.post("http://football-cards_service-4:5004/player", json=information)
